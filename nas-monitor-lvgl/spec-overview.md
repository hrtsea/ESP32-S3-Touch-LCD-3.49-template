...
│   ├── 蓝牙 / WiFi 图标                   ← 平级图标节点
│   └── 分隔线                              ← 视觉分隔
├── meter-panel       (0,  35, 240×100)    ← 第2层左：仪表盘
│   ├── meter-cpu      (12, 40, 90×90)     ← 圆弧仪表
│   └── meter-temp     (138,40, 90×90)     ← 圆弧仪表
├── metrics-bar       (260,35, 380×100)    ← 第2层右：指标条
│   ├── mem-container                       ← 内存进度条
│   └── disk-container                      ← 磁盘进度条
└── disk-strip        (0,  135,640×35)     ← 第3层：底部磁盘条
...
y=35  ├──────────────────────────────────┤  ← 分隔线 2px
      │ meter-panel    │  metrics-bar    │  ← 仪表 + 指标区
      │   (240px)      │   (380px)       │     (100px)
y=135 ├──────────────────────────────────┤
      │          disk-strip (35px)        │  ← 磁盘列表区
y=170 ├──────────────────────────────────┤
      │          bottom margin 2px        │
y=172 └──────────────────────────────────┘
```
...
meter-panel 与 metrics-bar 之间存在 20px 间隙 (240 + 20 = 260)，保证视觉呼吸感。
